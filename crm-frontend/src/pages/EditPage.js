...
const EditPage = () => {
  ...
  const handleTotalChange = (index, value) => {
    ...
    setFormState(prev => ({
      ...
      basicInfo: {
        ...prev.basicInfo,
        total: newTotal // Sync total with basicInfo
      }
  ...
  const addServiceToTable = (service) => {
    const cleanWorkdone = () => {
      ...
      const items = Array.from(description.props.children).map(item => 
  ...
  const formattedData = [
      ...
      formatColumns('Car Details:', formState.cars.map(car => `${car.carBrand} ${car.carModel} ${car.year} ${car.fuel}`).join(', ')),
